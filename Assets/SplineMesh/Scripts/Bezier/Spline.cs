...
namespace SplineMesh {
    ...
    public class Spline : MonoBehaviour {
        ...
        //Function to move spline node with knife
        float prevValuePos;
        float prevValueDir;
        float diff;
        int j;
        private void UpdateNodePosWithKnife()
        {
            
            if (HorizonStepManager.Inst.cutState == HorizonStepManager.HorizontalCuttingStates.cutting && gameObject.name=="Top")
            {
                diff = HorizonStepManager.Inst.cheeseSlicer.transform.position.x - prevValuePos;
                if (nodes[1].Position.z >= (nodes[0].Position.z+.90f))
                {
                    j = 1;
                    ...
                    for (; j < (nodes.Count ); j++)
                    {
                        nodes[j].Position = new Vector3(nodes[j].Position.x, nodes[j].Position.y, nodes[j].Position.z - diff);
                        nodes[j].Direction = new Vector3(nodes[j].Direction.x, nodes[j].Direction.y, nodes[j].Direction.z - diff);
                    }
                }
                //else
                {
                    //nodes[2].Position = new Vector3(nodes[2].Position.x, nodes[1].Position.y, nodes[2].Position.z);
                    //nodes[2].Direction = new Vector3(nodes[2].Direction.x, nodes[1].Direction.y, nodes[2].Direction.z );
                }
        ...
        //Initialising splines
        public void SplineInitialise()
        {
        ...
        private void Start()
        {
            Timer.Delay(0.5f, SplineInitialise);
            //SplineInitialise();
        }
