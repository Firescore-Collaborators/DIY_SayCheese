...
namespace SplineMesh
{
    ...
    {
        public float flyDurationInSecs = 10;
        ...
        private bool isMeshSliced = false;
        public bool IsMeshSliced
        {
            get { return isMeshSliced; }
            set
            {
                isMeshSliced = value;
                SetDirty();
        ...
        private void OnEnable()
        {
            ...
            {
                result = GetComponent<MeshFilter>().mesh;
            }
            else
            {
                GetComponent<MeshFilter>().mesh = result = new Mesh();
       ...
       //Updating vertex data of sliced mesh
        public void UpdateSourceVertexData(int index,float vertexYvalue)
        {   
            for (int i = 0; i < source.Vertices.Count; i++)
            {
                if (i == index)
                {
                    source.Vertices[i].position = new Vector3(source.Vertices[i].position.x, vertexYvalue, source.Vertices[i].position.z);
                    break;
                }
            }
        ...
        public void Contort()
        {
            ...
            {
                SetInterval(spline, spline.Length * rate);
                //ComputeIfNeeded();
            }
        }
        private void LateUpdate()
        {
            if (HorizonStepManager.Inst.cutState == HorizonStepManager.HorizontalCuttingStates.cutComplete)
            {
                //Debug.Log("Inside mesh bender");
                rate += Time.deltaTime / flyDurationInSecs;
                if (rate > 1)
                {
                ...
                //Debug.Log(" Rate = "+rate);
                if(rate>0.2 && speedUpdated ==false)
                {
                    speedUpdated = true;
                    flyDurationInSecs = .5f;
                }
                if (rate<.7f)
                {
                Contort();
                ...
                {
                    gameObject.GetComponent<MeshBender>().enabled = false;
                    HorizonStepManager.Inst.AfterCut();
                }
